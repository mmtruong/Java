 ...
 */
  public class Car
  {
   ...
   public void moveCar(){
       ...
       boolean inBoundary = ((xtobe + CAR_WIDTH + CAR_INCREMENT) < (win.getWidth()) 
       && ((ytobe - CAR_INCREMENT) > (0))
       && ((xtobe - CAR_INCREMENT) > (0))
       && ((ytobe + CAR_HEIGHT + CAR_INCREMENT - bottomLine)<(0)));
       
       boolean notOverlap = 
           (xtobe + CAR_WIDTH + CAR_INCREMENT) > (other.rrect.getX()) || 
           (ytobe - CAR_INCREMENT) > (other.rrect.getY() + CAR_HEIGHT)  || 
           (xtobe - CAR_INCREMENT) > (other.rrect.getX() + CAR_WIDTH ) || 
           (ytobe + CAR_HEIGHT + CAR_INCREMENT) < (other.rrect.getY());
           
        
        if( inBoundary && notOverlap){
         switch ( direction ){
         
         case RIGHT:
         transX=CAR_INCREMENT;   
         break;
           
               
         case UP:
         transY=-CAR_INCREMENT;
         break;
          ...
          break; 
            ...
            this.rrect.translate(transX,transY);
            win.repaint();
            }
        }
